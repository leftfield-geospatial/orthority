...
def downsample_dem(src_file: Path, dst_file: Path, ds_fact: int = 8, bounds: Tuple = None, dst_crs: rio.CRS = None):
    """
    ...
    with (rio.Env(GDAL_NUM_THREADS='ALL_CPUS', GTIFF_FORCE_RGBA=False), rio.open(src_file, 'r') as src_im):
        ...
        dst_array, dst_transform = reproject(
            src_array, src_crs=src_im.crs, src_transform=src_transform, src_nodata=src_im.nodata, dst_crs=dst_crs,
            dst_nodata=nodata, dst_resolution=(np.abs(src_transform.a) * ds_fact, ) * 2, resampling=Resampling.cubic,
            init_dest_nodata=True, num_threads=multiprocessing.cpu_count()
        )
        dst_profile = src_im.profile.copy()
        dst_profile.update(
            crs=dst_crs, width=dst_array.shape[-1], height=dst_array.shape[-2], transform=dst_transform,
            compress='deflate', interleave='band', tiled=True, blockxsize=256, blockysize=256, dtype='float32',
            driver='GTiff', predictor=2, zlevel=9, nodata=nodata
        )
        with rio.open(dst_file, 'w', **dst_profile) as dst_im:

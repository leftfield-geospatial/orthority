...
class OrthoIm:
    ...
    def __init__(
        ...
        ortho_filename: Union[str, Path] = None, config: Dict = None,
    ):
        """
        ...
        ortho_filename: str, pathlib.Path, optional
            Name of the orthorectified file to create.  If not specified, the `src_filename` is used with an '_ORTHO'
            postfix.
        ...
        tile_size: list of int, optional
        ...
        if not os.path.exists(src_filename):
            raise Exception(f"Source image file {src_filename} does not exist")
        ...
        self._dem_filename = Path(dem_filename)

        self._camera = camera

        if ortho_filename is None:
            self._ortho_filename = self._src_filename.parent.joinpath(
                self._src_filename.stem + '_ORTHO' + self._src_filename.suffix
            )
        else:
            self._ortho_filename = Path(ortho_filename)
        ...
        logger.debug(f'DEM: {self._dem_filename.name}')

    def _check_rasters(self):
        """
        Check that the source image is not 12 bit
        """
        with rio.Env(GDAL_NUM_THREADS='ALL_CPUs'), suppress_no_georef(), rio.open(self._src_filename, 'r') as src_im:
                try:
                    ...
                    if src_im.profile['compress'] == 'jpeg':  # assume it is a 12bit JPEG
                        raise Exception(
                            f'Could not read {self._src_filename.stem}\n'
    ...
    def _parse_config(self, config):
        ...
        # TODO: can the use of the source CRS be dropped entirely?  Also, can opening the source image many times be
        #  avoided?
        ...
        if self._ortho_filename.exists():
            if self.overwrite:
                logger.warning(f'Deleting existing ortho file: {self._ortho_filename.stem}.')
                os.remove(self._ortho_filename)
            else:
                raise FileExistsError(f'Ortho file {self._ortho_filename.stem} exists, skipping.')

    def _get_init_dem(self) -> Tuple[np.ndarray, rio.Affine]:
        """
        Return an initial DEM array and corresponding transform, in the ortho CRS and resolution.

        The DEM array is created to cover the ortho bounds at the z=min(DEM) plane, accounting for worst case
        vertical datum offsets between the DEM and ortho CRS, and within the limits of the DEM image bounds
        themselves.
        """
        with rio.Env(GDAL_NUM_THREADS='ALL_CPUs'), rio.open(self._dem_filename, 'r') as dem_im:
            ...
            def get_win_at_z_min(z_min: float) -> Window:
                ...
                try:
                    ...
                    raise ValueError(f'Ortho {self._ortho_filename.name} lies outside DEM {self._dem_filename.name}')
        ...
        dem_array, dem_transform = reproject(
            ...
            dst_crs=self.crs, dst_resolution=self.resolution, resampling=self.dem_interp, dst_nodata=float('nan'),
    ...
    def _get_ortho_poly(self, dem_array: np.ndarray, dem_transform: rio.Affine, num_pts=400) -> np.ndarray:
        ...
        for pi in range(src_ji.shape[1]):
            ...
            # find the dem pixel co-ords, and validity mask for the (x, y) points in ray_xyz
            ...
            if not np.any(mask):
                ...
                # ray_xyz lies at least partially inside the dem (x, y) bounds, but may not not have an intersection
    ...
    def _poly_mask_dem(
        ...
        if np.all(poly_min > dem_bounds[-2:]) or np.all(poly_max < dem_bounds[:2]):
            ...
            logger.warning(
                f'Ortho {self._ortho_filename.name} is not completely covered by DEM {self._dem_filename.name}'
            )
        # clip to dem bounds
    ...
    def _remap_src_to_ortho(self, ortho_profile: Dict, dem_array: np.ndarray):
        """
        ...
        with env, suppress_no_georef(), rio.open(self._src_filename, 'r') as src_im:
            with rio.open(self._ortho_filename, 'w', **ortho_profile) as ortho_im:
                ...
                for bi in bands.tolist():
                    ...
                    for ji, ortho_win in ortho_im.block_windows(1):
                        ...
                        if (self.interp != CvInterp.nearest and not np.isnan(self.nodata) and
    ...
    def _build_ortho_overviews(self, max_num_levels: int = 8, min_level_pixels: int = 256):
        """ Build internal overviews for the ortho image. """
        if self._ortho_filename.exists():  # build internal overviews
            with rio.Env(GDAL_NUM_THREADS='ALL_CPUs'):
                with rio.open(self._ortho_filename, 'r+') as ortho_im:
